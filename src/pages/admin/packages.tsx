...
const PackageManagement: React.FC = () => {
  const { isAuthenticated, isLoading, isAdmin, adminChecked } = useRequireAdmin('/unauthorized');
  const [packages, setPackages] = useState<Package[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string>('');
  const [searchTerm, setSearchTerm] = useState('');
  ...
  const fetchPackages = useCallback(async () => {
    try {
      ...
      const errorMessage = fetchError instanceof Error ? fetchError.message : 'Failed to load packages';
  ...
  }, [adminChecked, isAdmin, isAuthenticated, fetchPackages]);
  ...
  const handleStatusToggle = async (packageId: string, currentStatus: boolean) => {
    try {
      ...
      console.error('❌ Failed to update package status:', statusError);
      const errorMessage = statusError instanceof Error ? statusError.message : 'Failed to update package status';
      setError(errorMessage);
    }
  };
  ...
  const handleFeaturedToggle = async (packageId: string, currentStatus: boolean) => {
    try {
      ...
      console.log('✅ Featured status updated successfully');
    } catch (featuredError: unknown) {
      console.error('❌ Failed to update featured status:', featuredError);
      const errorMessage = featuredError instanceof Error ? featuredError.message : 'Failed to update featured status';
      setError(errorMessage);
    }
  };
  ...
  const handleDeletePackage = async (packageId: string) => {
    ...
    try {
      ...
      const errorMessage = deleteError instanceof Error ? deleteError.message : 'Failed to delete package';
      setError(errorMessage);
    }
  };
