...
// // Server-side validation function
// const validateSignupServer = (data: SignupData) => {
//   const errors: string[] = [];
...
//       const messages = Object.values((error as ValidationError).errors).map((err) => err.message);
//       return res.status(400).json({ 
//         success: false,
//         message: messages[0] || 'Validation failed',
//         errors: messages
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const userData = {
      ...
      verificationTokenExpires: emailVerificationExpiry,
      isVerified: false,
      role: 'customer'
    };
    ...
    // Send welcome email using emailService (don't fail signup if email fails)
    try {
      const firstName = cleanName.split(' ')[0]; // Get first name
      await emailService.sendWelcomeEmail(cleanEmail, firstName);
      console.log('Welcome email sent successfully to:', cleanEmail);
    } catch (emailError) {
      console.error('Error sending welcome email:', emailError);
      // Continue with successful signup even if email fails
