...
//         console.log(`✅ Package found: ${packageData.title}`);
//         res.status(200).json({ success: true, package: packageData });
//       } catch (error: mongoose.Error | Error) {
//         console.error('❌ GET package error:', error);
...
//             success: false,
//             message: 'Validation error',
//             errors: Object.values(error.errors).map((err: mongoose.Error.ValidatorError) => err.message)
//           });
//         }
      ...
      try {
        ...
        if (error instanceof mongoose.Error.ValidationError) {
          return res.status(400).json({
            success: false,
            message: 'Validation error',
            errors: Object.values(error.errors).map((err) => (err as mongoose.Error.ValidatorError).message)
          });
        }
