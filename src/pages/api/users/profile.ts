...
// // Interface for update data to avoid using 'any'
// interface UpdateData {
//   name: string;
//   phone?: string;
//   dateOfBirth?: Date;
//   gender?: string;
//   address?: any; // Replace with specific type if address structure is known
//   preferences?: any; // Replace with specific type if preferences structure is known
//   updatedAt: Date;
// }
...
//           success: false,
//           message: 'Validation error',
//           errors: Object.values(error.errors).map((e: mongoose.Error.ValidatorError) => e.message)
//         });
//       }
...
// // Interface for preferences structure
  ...
  return authenticate(async (req: AuthenticatedRequest, res: NextApiResponse) => {
    try {
      ...
      if (req.method === 'GET') {
        ...
        // Get user profile with all details except password
        const user = await User.findById(req.user?._id).select('-password').lean() as UserProfile | null;
      ...
      if (error instanceof mongoose.Error.ValidationError) {
        return res.status(400).json({
          success: false,
          message: 'Validation error',
          errors: Object.values(error.errors).map((e) => (e as mongoose.Error.ValidatorError).message)
        });
      }
