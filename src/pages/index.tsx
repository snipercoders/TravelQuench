...
//               <Card key={category.title} padding="none" className="overflow-hidden hover:shadow-xl transition-all duration-300 group cursor-pointer border-0 rounded-xl sm:rounded-2xl">
//                 <div className={`h-24 sm:h-28 lg:h-32 bg-gradient-to-br ${category.color} relative overflow-hidden`}>
//                   <div className="absolute inset-0 bg-black/10 group-hover:bg-black/20 transition-all duration-300"></div>
//                   <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
//                     <category.icon className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 mb-1 sm:mb-2 group-hover:scale-110 transition-transform duration-300" />
...
//               <div className="text-center mb-6 sm:mb-8">
//                 <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Get Your Custom Quote</h3>
//                 <p className="text-gray-600 text-sm sm:text-base">Fill out the form and we will get back to you instantly</p>
//               </div>
...
const getScrollAmount = (ref: React.RefObject<HTMLDivElement | null>) => {
  if (!ref.current) return 300; // Default scroll amount if ref is null
  const containerWidth = ref.current.offsetWidth;
  return isMobile ? containerWidth * 0.85 : containerWidth * 0.5;
};
